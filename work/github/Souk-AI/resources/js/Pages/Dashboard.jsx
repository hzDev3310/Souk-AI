...
import ClientDashboard from './Roles/Client/Dashboard';
import StoreDashboard from './Roles/Store/Dashboard';
import InfluencerDashboard from './Roles/Influencer/Dashboard';
import ShippingCompanyDashboard from './Roles/ShippingCompany/Dashboard';
import ShippingEmployeeDashboard from './Roles/ShippingEmployee/Dashboard';
...
export default function Dashboard({ auth }) {
    ...
    const renderDashboard = () => {
        switch (role) {
            ...
            case 'STORE':
                return <StoreDashboard auth={auth} />;
            case 'CLIENT':
                return <ClientDashboard auth={auth} />;
            case 'INFLUENCER':
                return <InfluencerDashboard auth={auth} />;
            case 'SHIPPING_COMPANY':
                return <ShippingCompanyDashboard auth={auth} />;
            case 'SHIPPING_EMP':
                return <ShippingEmployeeDashboard auth={auth} />;
            default:
                // Default fallback to generic dashboard (the original content)
                return (
    ...
    const getHeaderTitle = () => {
        switch (role) {
            ...
            case 'STORE': return 'Merchant Hub';
            case 'CLIENT': return 'My Souk Dashboard';
            case 'INFLUENCER': return 'Creator Studio';
            case 'SHIPPING_COMPANY': return 'Logistics Control';
            case 'SHIPPING_EMP': return 'Delivery Portal';
            default: return 'Curator Dashboard';
        }
    };
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-bold text-3xl text-secondary dark:text-white leading-tight">{getHeaderTitle()}</h2>}
        >
